...
.Der Graph ist ein Histogramm, welches die Sortierschritte pro produktiven Schritt von allen Losen in Klassen unterteilt und in Form von Balken darstellt. Je höher der Balken, desto mehr Lose sind in der jeweiligen Klasse (Sortierschritten pro produktive Schritte) gelandet. Als Vergleich wurde mit dem _roten_ Graphen eine möglichst ähnliche Gaussche Verteilung herangezogen.
...
.*ROUTE* bezeichnet dabei den einzigartigen Identifier der Route und *COUNT* von wie vielen Lots die Route in unserem Datensatz durchfahren wird. Die Tabelle ist nach der COUNT-Spalte absteigend sortiert und umfasst die Routen der 975 Lose mit den höchsten Sortierschritten pro Produktivschritt.
[%header, format=csv]
|===
